...
By default, Bun limits the number of simultaneous `fetch` requests to 256 per origin (scheme + hostname + port), with a process-wide ceiling of four times that (1,024) across all origins, for two reasons:
...
To change the per-origin limit, set the `BUN_CONFIG_MAX_HTTP_REQUESTS` environment variable; the process-wide ceiling is always four times it:

```sh
BUN_CONFIG_MAX_HTTP_REQUESTS=512 bun ./my-script.ts
```

The max value for this limit is 65,535.
