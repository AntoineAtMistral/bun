...
Bun automatically decodes percent-encoded route parameter values, including Unicode characters. Invalid Unicode is replaced with the replacement character `&0xFFFD;`. A `%` that is not followed by two hexadecimal digits is not an escape and is preserved literally.
