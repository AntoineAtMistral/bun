...
impl VM {
    ...
    /// pointer (`JSC::sanitizeStackForVM`). The event loop calls this before
    /// dispatching a batch of JS callbacks so the dispatch frames are not built
    /// over stale JSValues the conservative GC scan would treat as roots.
