...
#if !OS(WINDOWS)
// Records every fd whose FD_CLOEXEC bit execve() changes so a failed
// execve(2) can put them back: the failure now throws back to JS (Node
// semantics), and Node likewise restores the flags it changed.
...
JSC_DEFINE_HOST_FUNCTION(Process_functionExecve, (JSGlobalObject * lexicalGlobalObject, CallFrame* callFrame))
{
    ...
    // into the replacement process (recorded so a failed execve restores them).
    {
        ...
        for (int fd = 3; fd < maxfd; fd++) {
            ...
            if (flags >= 0 && !(flags & FD_CLOEXEC))
                cloexecRestorer.setFlags(fd, flags, flags | FD_CLOEXEC);
        }
    }
    ...
    // undo the FD_CLOEXEC changes (Node does the same) and throw Node's
    // SystemError carrying the syscall, errno, and path.
