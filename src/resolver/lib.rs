...
pub mod fs {
    ...
    impl RealFS {
        ...
        pub fn entries_at(
            ...
            // Callers are mixed: `dir_info_uncached` reaches this under
            // `entries_mutex` (via `dir_info_cached_miss`), while the
            // `DirInfo::get_entries*` accessors do not take it.
            let result_ptr = std::ptr::from_mut::<EntriesOption>(self.entries.at_index(index)?);
            // SAFETY: BSSMap-owned slot; see the caller-contract note above.
            if let EntriesOption::Entries(existing) = unsafe { &mut *result_ptr } {
                // NOTE: deliberately ignores `DirEntry.stale`. This accessor serves
                // `DirInfo::get_entries*` readers that run without `entries_mutex`
                // (and on other threads), so rebuilding here would free the map out
                // from under them. A stale listing is refreshed by the next locked
                // directory read (`read_directory_with_iterator`,
                // `dir_info_cached_miss`, `dir_info_for_resolution`); until then
                // these readers see their `DirInfo`'s listing, exactly as they saw
                // the orphaned one before the bust kept the slot.
                if existing.generation < generation {
                    let e_ptr: *mut DirEntry = std::ptr::from_mut::<DirEntry>(*existing);
                    // SAFETY: BSSMap-owned `DirEntry` (boxed/leaked into `EntriesOption`).
