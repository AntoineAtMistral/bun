...
describe("node:test", () => {
  ...
  test("should forward an Infinity timeout so it overrides the runner default", async () => {
    const { exitCode, stderr } = await runTests(["11-timeout-infinity.js"], {}, ["--timeout", "100"]);
    expect(stderr).toContain("1 pass");
    expect({ exitCode, stderr }).toMatchObject({
      exitCode: 0,
      stderr: expect.stringContaining("0 fail"),
